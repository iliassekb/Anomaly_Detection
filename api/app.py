...
def annotate_frame(frame, result, class_names, conf_thr, product_mask=None, mask_overlap_thr=0.20):
    ...
    for i in range(len(boxes)):
        ...
        detections.append({"class": name, "class_id": cls_id, "conf": round(conf, 4)})
        overlay[mask_bin == 1] = (0, 0, 255)
        if smooth_cnt is not None:
            cv2.polylines(frame, [smooth_cnt], True, (0, 0, 255), 2, cv2.LINE_AA)
        else:
...
@app.get("/api/models")
def list_models():
    candidates = ["best_m.pt", "best_s.pt"]
